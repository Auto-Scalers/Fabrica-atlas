# Orchestration Ideas — Categorized by Sub-System

> **Purpose:** Extract every orchestration-relevant idea/feature from the Fabrica-atlas discovery & analysis files and bucket it under the 12 orchestration sub-systems. Each bullet is tagged `[Fabrica]` / `[MC]` / `[buzz]` with a `file:line` (or section) citation. `[Fabrica]` = what already exists (baseline to preserve); `[MC]` / `[buzz]` = adoptable reference designs.
>
> **Companion doc:** `orchestration-system.md` (verified mapping of the same 12 titles into Fabrica's actual source). This file adds the *idea buckets* — what to keep, what to port, what to converge.
>
> **Sources scanned:** `discovery/fabrica-app-discovery.md`, `discovery/Fabrica-features.md` (UTF-16, read via UTF8), `discovery/mc-features.md`, `discovery/buzz-features.md`, `discovery/mission-control-discovery.md`, `discovery/buzz-discovery.md`, `discovery/mission-control/*.md` (mc-decision-gates, mc-workflow-engine, mc-execute-guards, mc-chainedispatch-reconciler, mc-fieldtask-kanban, mc-adapters-linelevel, mc-ai-providers, mc-frontend-buzz-clients), `discovery/buzz/*.md` (buzz-agent-crates, buzz-desktop, bz-relay-event-kinds, bz-db-schema, bz-ops-deploy-admin, bz-search-pubsub, bz-voice-media, bz-pair-relay-cli), `discovery/fabrica-app/*.md` (fa-agent-hooks-probes, fa-plugin-runtime), `analysis/*.md` (production-architecture, r5-agent-platform-integration-map, r5-convergence-memo, similarities-gaps, round4-findings-digest).
>
> **Hard constraint:** preserve every existing Fabrica feature (Fabrica-App Transformation Rule). Enhance/extend only.
