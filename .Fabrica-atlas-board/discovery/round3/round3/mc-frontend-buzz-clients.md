...
# Discovery Report â€” Mission-Control Frontend + Buzz Mobile/Web Clients

Task: task_b1957c7492d3 / dispatch ctx_df87c62c67ff â€” READ-ONLY discovery.
All paths relative to `C:\Users\BAB AL SAFA\Desktop\Fabrica-development_environment\_sources\`.

---

## AREA 1 â€” mission-control frontend (`mission-control/mission-control/src`)
...
**`hooks/use-data.ts` â€” `useDataResource<T>(endpoint, dataKey, label, pollInterval?)`**
Generic CRUD factory (lines 10â€“217):
...
- Polling: optional interval; polls only when `document.visibilityState === "visible"`; immediate refetch on tab re-visible (lines 40â€“58).
- `create`: POST, appends to state, success toast.
- `update`: **optimistic update**, revert via refetch on failure (lines 81â€“105).
- `remove`: optimistic delete + **undo toast with 5s window** that PUTs `{id, deletedAt:null}` to restore soft-deleted item (lines 107â€“148).
- `bulkUpdate`/`bulkRemove`: for endpoint `tasks` uses single atomic `/api/tasks/bulk` PUT/DELETE; other entities fall back to parallel individual calls (lines 150â€“214).
...
**`hooks/use-field-ops.ts` â€” mirrors the same pattern (`useFieldOpsResource`, lines 10â€“128)** minus bulk ops and undo:
- `useFieldMissions` â†’ `/api/field-ops/missions`, 15s poll.
- `useFieldTasks` â†’ `/api/field-ops/tasks`, 10s poll.
- `useFieldServices` â†’ `/api/field-ops/services`, no poll.

**Client-side password cache (module-level singleton, lines 130â€“163):**
...
**`useVaultSession()` (lines 173â€“250):**
...
### 1.2 `app/field-ops/page.tsx` â€” dashboard

- Loads 5 endpoints in parallel on mount: missions, tasks, services, activity(?limit=10), approval-config (lines 206â€“245). Errors swallowed silently (empty-state rendering).
- **Autonomy modes** (`AUTONOMY_MODES`, lines 61â€“83): `approve-all` (Manual Approval, emerald), `approve-high-risk` (Supervised, amber), `full-autonomy` (Full Autonomy, red + `animate-pulse` when active).
- **Autonomy change flow**: click mode â†’ `pendingMode` set â†’ master-password dialog opens (lines 251â€“299). Confirm sends PUT `/api/field-ops/approval-config` with `{mode, masterPassword}`, `retries:0`. Full-autonomy shows a red warning banner about financial transactions. On success refreshes activity feed. Dialog cancel resets all pending state.
...
- FinancialOverviewCard (`variant="detailed"`), GettingStartedCard (storageKey `mc-fieldops-dashboard-intro`), active-mission cards with per-mission progress bar and pending-count badge, recent activity list with full event-type label/color maps (21 event types, lines 87â€“135).

### 1.3 `app/field-ops/approvals/page.tsx` â€” batch approval UX

- Loads only `tasks?status=pending-approval` + services + missions in parallel (lines 47â€“72).
- **Risk classification client-side** (`getTaskRisk`, lines 80â€“86): high = payment/crypto-transfer/ad-campaign; medium = email-campaign/social-post/publish; else low. Three clickable stat cards act as risk filters (`riskFilter` toggle).
- **Selection model**: `selectedIds: Set<string>`; per-task checkbox + select-all (scoped to filtered set).
- **Batch approve**: POST `/api/field-ops/batch` `{action:"approve", taskIds, actor:"me"}`; reports `succeeded.length` and `failed.length` in toast (lines 173â€“200).
- **Batch reject**: same batch endpoint with `rejectionFeedback`; triggered through `RejectTaskDialog` using a sentinel task `{id:"__batch__"}` so one dialog handles single + batch (lines 426, 498â€“515).
...
### 1.4 `app/field-ops/vault/page.tsx` â€” vault management

- Fetches credentials list + health (`GET /vault` and `GET /vault?health=true`) in parallel (lines 291â€“310). UI only ever sees **metadata** (`CredentialMeta{id, serviceId, createdAt, expiresAt}`) â€” secrets never returned by API.
- **Health-driven rendering**:
  - `masterKeyFormat === "none"` â†’ `VaultSetupWizard` (initial setup).
  - `legacyCredentials > 0` â†’ yellow "Migration Required" banner (storing a new credential auto-upgrades all to AES-256-GCM).
  - Healthy â†’ green "AES-256-GCM Encryption Active" banner.
- **VaultSessionCard** (lines 165â€“274): unlock form when locked; when active shows green card with countdown (local 10s decrement of `remainingMs` from `useVaultSession`) and "Lock Now". Unlocking "caches your password in memory for 30 minutes."
- Add credential inline form: serviceId + apiKey + masterPassword â†’ POST `/api/field-ops/vault` `{serviceId, data, masterPassword}`.
- Revoke: DELETE `/api/field-ops/vault?id=...`.
- **Reset vault**: destructive dialog requiring typed `RESET` confirmation â†’ POST `/api/field-ops/vault/reset` `{confirm:"RESET_VAULT"}` (lines 641â€“699).
...
- Pending-action state: `pendingExecuteTask`, `pendingDryRun`, `pendingAction {taskId,status,feedback?}` (lines 308â€“311).
- `handleStatusChange` (line 404): approving requires vault auth â€” if not active, defer into `pendingAction` + open dialog.
...
- **Circuit breaker (ASI08)**: `shouldTripCircuitBreaker(taskStatuses)` from `lib/field-ops-security.ts` â€” â‰¥3 consecutive failures trips a red warning card with "Pause Mission" button (lines 337â€“338, 693â€“716).
...
## AREA 2 â€” Buzz mobile relay transport (`buzz/mobile/lib`)

Flutter + Riverpod + hooks. Relay stack lives in `shared/relay/`, features in `features/`.

### 2.1 `shared/relay/relay_socket.dart` â€” low-level WebSocket + NIP-42
...
- AUTH challenge handling (`_handleAuthChallenge`, lines 195â€“231): decodes bech32 nsec â†’ hex privkey, builds kind:22242 event with tags `[relay, wsUrl]` + `[challenge, ...]`, sends `["AUTH", event]`, tracks `_pendingAuthEventId`.
- OK frames matching the pending auth id complete/fail the auth completer; rejection messages classified by `classifyRelayAuthFailure` (`error:` prefix â†’ generic Exception, else `RelayAuthRejectedException`). All other frames (EVENT/EOSE/NOTICE/OK) forwarded upstream. Binary frames and malformed JSON ignored.
- No reconnection logic here â€” explicitly delegated to `RelaySessionNotifier`.

### 2.2 `shared/relay/relay_session.dart` â€” session manager (Riverpod Notifier)

`RelaySessionNotifier extends Notifier<SessionState>` (state = status + reconnectAttempt). Auto-connects via microtask when authenticated AND config has nsec (build(), lines 146â€“164).

Key internals:
- **Reconnect**: exponential backoff 1sâ†’30s max (`_baseReconnectDelayMs=1000`, `_maxReconnectDelayMs=30000`). Connection generations (`_connectionGeneration`) invalidate stale callbacks.
- **Live subscription replay**: on reconnect, REQs re-sent with `since: lastSeenCreatedAt - 5s` skew (`_reconnectReplaySkewSeconds`), batched 8 at a time with 50ms inter-batch delay; visible channel (registered via `registerVisibleChannel`, owner-scoped release callback) is prioritized in replay ordering (lines 512â€“532).
...
- **History queries**: `fetchHistory(filter)` â€” one-shot REQ until EOSE, 8s timeout, CLOSE sent afterwards.
- **Publish**: `publish(event)` registers `_PendingEvent` keyed by event id, resolves on OK (accepted â†’ minimal placeholder NostrEvent whose `content` carries the OK message, e.g. `response:{...}` for command kinds like 41010/30620/46020; rejected â†’ error). 8s ack timeout.
- **CLOSED handling** (`_handleClosed`, lines 666â€“747): classifies via `relay_closed_policy.dart` (`classifyRelayClosed` â†’ terminal vs rateLimited). Terminal â†’ fail ready-completer + remove sub. Rate-limited â†’ activates rate-limit gate and retries with backoff (attempt-capped exponential, floor of gate remaining time). Retries go through `_pendingClosedRetries` + microtask-scheduled replay.
- **Lifecycle**: `onAppPaused` starts a 5s grace timer then pauses (disconnect, cancel history/pending); `onAppResumed` reconnects immediately (cancels backoff) if backgrounded â‰¥ grace period or not connected.
- **HTTP bridge query**: `queryRelay(filters)` â†’ POST `/query` with NIP-98 Authorization header built by local `buildNip98AuthHeader` (kind:27235 event, tags u/method/payload(SHA-256 hex of body)/nonce(uuid), base64-encoded, lines 949â€“979). Non-2xx triggers rate-limit-gate activation from error body.
...
- Auth: BUD-01 style kind:**24242** event with tags `t=upload`, `x=<sha256>`, `expiration=now+300s`, optional `server=<authority>`; header `Nostr <base64url(no padding)>` plus `X-SHA-256` (lines 573â€“620). Download-side auth in `media_auth.dart` (kind 24242, `t=get`).
- Preparation pipeline: magic-byte MIME detection; HEIC brand detection â†’ transcode to JPEG; animated GIF/PNG/WebP sanitized via `animated_image_sanitizer.dart`; video transcoded to MP4 (fast-start via `mp4_fast_start.dart`) and poster frame generated â€” heavy work bridged through platform channel `buzz/media_upload` (native methods sanitizeImageForUpload/transcodeVideoToMp4/generateVideoPoster/transcodeImageToJpeg/readClipboardImage etc.).
...
Large feature module (~120 files), organized as page + sibling `part` folders per repo convention (one public widget per file, â‰¤1000 lines/file):
...
- **`pairing_provider.dart`** (969 lines, `PairingNotifier`): statuses idleâ†’connectingâ†’confirmingSasâ†’transferringâ†’storingâ†’success/error.
  - Flow (`_pairNipAb`): parse QR â†’ generate ephemeral keypair â†’ derive sessionId+SAS immediately â†’ precompute NIP-44 conversation key (`shared/crypto/nip44.dart`) â†’ connect `PairingSocket` (ephemeral key) â†’ subscribe kind:**24134** tagged to ephemeral pubkey â†’ publish encrypted `offer {version, session_id}` â†’ show SAS code â†’ 120s session timeout.
  - Source sends `sas-confirm` carrying transcript hash; target verifies with constant-time compare; mismatch â†’ abort `sas_mismatch` ("possible attack").
...
## AREA 3 â€” Buzz web client (`buzz/web/src`)
...
- `ensureClone(owner, repo, ref)`: stat `.git` in virtual FS â†’ existing ? shallow `fetch` (depth 1, singleBranch; failures tolerated) : shallow `clone` (depth 1, singleBranch, noTags). Returns `{fs, dir}` consumed by all hooks.
- `readTreeEntries` (tree listing), `readFileContent` (binary detect via NUL scan of first 512 bytes), `readBlobView` â†’ discriminated `BlobView`: image (raster ext map incl. avif; SVG deliberately excluded for active-content safety) capped at 10 MiB; binary (NUL sniff OR fatal UTF-8 decode failure); markdown/html/text capped at 1 MiB; `too-large` carries which limit was hit. Display caps only â€” IndexedDB clone holds full bytes. Object URLs deliberately NOT created inside React Query cache (viewer owns createObjectURL lifecycle).
- `resolveHtmlAssets`: for HTML preview "Run" â€” inlines only same-repo *relative* script/link/img refs as `data:` URLs (external/absolute/fragment refs untouched; `..` cannot escape repo root); result rendered in sandboxed iframe.
- `getCommitLog` (depth 20 default), `findReadme` (readme.md/readme/readme.rst/readme.txt priority).

**`features/repos/use-git-browse.ts`** (143 lines): TanStack Query hooks all depending on `useGitClone` (queryKey `["git-clone", owner, repo, ref]`, staleTime 5 min, retry false, enabled-guarded): `useGitTree` (dirs-first sort), `useGitLog`, `useGitReadme`, `useGitBlob`, `useGitHtmlDoc` (lazy â€” `enabled` caller-gated so asset inlining only happens on user "Run").
...
- Signs via NIP-07 (`window.nostr`) when present, with integrity checks: returned pubkey must match `getPublicKey()`, and signed event must be byte-identical to the unsigned template (kind/created_at/content/tags compare) â€” else "extension returned an invalid signed event".
...
**`shared/lib/nip98.ts`** (48 lines): `makeNip98AuthHeader(url, method, {body?, requireNip07?})` â€” kind:27235 event, tags `u`, `method`, plus `payload` (SHA-256 hex of body, required by invite endpoints) and `nonce` (crypto.randomUUID()) when body present; base64(JSON) â†’ `Nostr <b64>` header.

**`shared/lib/nostr-client.ts`** (175 lines): one-shot `queryEvents(wsUrl, filter)` â€” opens WS, waits up to 100ms for an AUTH challenge before sending REQ (Buzz relays always challenge; others may not); NIP-42 response via `makeAuthEvent` (nostr-tools/nip42) + signer; OK-for-auth-event gates REQ; collects EVENTs until EOSE; CLOSED â†’ reject; 10s overall timeout; resolves partial events on clean close.

### 3.3 Same-origin relay serving

**`shared/lib/relay-url.ts`** (24 lines): `VITE_RELAY_URL` env override; otherwise derive from `window.location` (httpsâ†’wss, httpâ†’ws) â€” i.e., the web bundle expects to be served BY the relay host, making WS + HTTP (git smart transport, `/query`, Blossom media) same-origin by default. `relayHttpBaseUrl()` converts WSâ†’HTTP scheme.
...
1. **Three parallel NIP-98 implementations** exist: mobile Dart (`relay_session.dart buildNip98AuthHeader` â€” SHA-256 payload tag always), web TS (`nip98.ts` â€” payload+nonce only when body present), and desktop (not inspected here). Tag-shape differences could matter for relay-side verification strictness.
